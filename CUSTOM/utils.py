...
from paddle.vision import utils as paddle_utils

def check_positive(am):
    am = paddle.to_tensor(am)
    am = paddle.logical_and(am > 0.5, paddle.full_like(am, 1))
    am = paddle.logical_or(am <= 0.5, paddle.full_like(am, 0))
    edge_mean = (am[0, 0, 0, :].mean() + am[0, 0, :, 0].mean() + am[0, 0, -1, :].mean() + am[0, 0, :, -1].mean()) / 4
    return edge_mean.numpy() > 0.5

imagenet_mean = [0.485, 0.456, 0.406]
imagenet_std = [0.229, 0.224, 0.225]

def model_info(model, log_func=print):
    n_p = sum(x.numpy().size for x in model.parameters())  # number parameters
    n_g = sum(x.numpy().size for x in model.parameters() if x.trainable)  # number gradients
    log_func('\n%5s %50s %9s %12s %20s %12s %12s' % ('layer', 'name', 'gradient', 'parameters', 'shape', 'mu', 'sigma'))
    for i, (name, p) in enumerate(model.named_parameters()):
        name = name.replace('module_list.', '')
        log_func('%5g %50s %9s %12g %20s %12.3g %12.3g' % (
            i, name, p.trainable, p.numpy().size, list(p.numpy().shape), p.mean().numpy(), p.std().numpy()))
...
def visualize_heatmap(experiments, images, attmaps, epoch, cnt, phase='train', seg=False):
    ...
    paddle_utils.save_image(paddle.to_tensor(images), './experiments/images/{}/{}/{}-{}-pri.jpg'.format(experiments, phase, epoch, cnt), nrow=n,
                            normalize=True)
    ...
    for j in range(axes.shape[0]):
        for k in range(axes.shape[1]):
            ...
            temp = paddle.to_tensor(temp)
            if temp.shape[0] != h:
                temp = cv2.resize(temp.numpy(), (w, h), interpolation=cv2.INTER_CUBIC)
            axes[j, k].imshow(temp.numpy())
            axes[j, k].axis('off')
    ...
    for j in range(axes.shape[0]):
        for k in range(axes.shape[1]):
            temp = attmaps[j * n + k, 0, :, :]
            temp = paddle.to_tensor(temp)
            axes[j, k].hist(temp.numpy().ravel(), bins=50, range=(0, 1), color='cornflowerblue')
    ...
    attmaps = paddle.to_tensor(attmaps).squeeze().numpy()
    ...
    for i in range(images.shape[0]):
        ...
        grid = paddle_utils.make_grid(paddle.to_tensor(images[i].unsqueeze(0)), nrow=1, padding=0, pad_value=0,
                              normalize=True, scale_each=False, range=None)
        # Add 0.5 after unnormalizing to [0, 255] to round to nearest integer
        image = grid.mul(255).add(0.5).clamp(0, 255).permute((1, 2, 0)).numpy().astype(np.uint8)[..., ::-1]
