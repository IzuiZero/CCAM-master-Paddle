...
from paddle.vision.transforms import Compose, Resize, RandomHorizontalFlip, Normalize, ToTensor
...
if __name__ == '__main__':
    ...
    model.prepare(optimizer=None, loss=criterion, metrics=None)
    model.cuda()
    model.train()
    # model_info(model)
    ...
    # Loss, Optimizer
    ###################################################################################
    criterion = [SimMaxLoss(metric='cos', alpha=args.alpha),
                 SimMinLoss(metric='cos')]
    ...
    for epoch in range(args.max_epoch):
        for iteration, (images, _) in enumerate(train_loader):
            ...
            train_meter.add({
                'loss': loss.numpy()[0],
                'positive_loss': loss1.numpy()[0],
                'negative_loss': loss2.numpy()[0],
            })
            ...
            if (iteration + 1) % 50 == 0:
                visualize_heatmap(args.tag, images.clone().detach(), ccam.numpy(), 0, iteration)
