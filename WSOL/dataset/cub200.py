...
class CUB200(paddle.io.Dataset):
    """
    CUB200 dataset.
    ...
    def __getitem__(self, index):
        ...
        Returns
        ----------
        image: paddle.Tensor, [3, H, W]
            Image of the given index.
        target: int
            Label of the given index.
        parts: paddle.Tensor, [15, 4]
            Landmark annotations.
        boxes: paddle.Tensor, [5, ]
        ...
        if np.random.random() <= 0.5 and self.iaa_aug:
            aug = self.iaa_augmentation()
            image = aug(image)

        # numpy arrays to paddle tensors
        parts = paddle.to_tensor(parts, dtype='float32')
    ...
    def iaa_augmentation(self):
        import imgaug.augmenters as iaa
        return iaa.CoarseDropout(0.2, size_percent=0.02)
