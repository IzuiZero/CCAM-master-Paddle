...
def to_2d_tensor(inp):
    ...
    if len(inp.shape) < 2:
        inp = paddle.unsqueeze(inp, axis=0)
    return inp
...
def compute_IoU(pred_box, gt_box):
    ...
    boxes1[:, 2] = paddle.clip(boxes1[:, 0] + boxes1[:, 2], 0, 1)
    boxes1[:, 3] = paddle.clip(boxes1[:, 1] + boxes1[:, 3], 0, 1)

    boxes2 = to_2d_tensor(gt_box)
    boxes2[:, 2] = paddle.clip(boxes2[:, 0] + boxes2[:, 2], 0, 1)
    boxes2[:, 3] = paddle.clip(boxes2[:, 1] + boxes2[:, 3], 0, 1)
    # boxes2 = xywh_to_x1y1x2y2(boxes2)

    intersec = boxes1.clone()
    intersec[:, 0] = paddle.maximum(boxes1[:, 0], boxes2[:, 0])
    intersec[:, 1] = paddle.maximum(boxes1[:, 1], boxes2[:, 1])
    intersec[:, 2] = paddle.minimum(boxes1[:, 2], boxes2[:, 2])
    intersec[:, 3] = paddle.minimum(boxes1[:, 3], boxes2[:, 3])

    def compute_area(boxes):
        # in (x1, y1, x2, y2) format
        dx = boxes[:, 2] - boxes[:, 0]
        dx = paddle.clip(dx, min=0)
        dy = boxes[:, 3] - boxes[:, 1]
        dy = paddle.clip(dy, min=0)
        return dx * dy

    a1 = compute_area(boxes1)
    a2 = compute_area(boxes2)
    ia = compute_area(intersec)
    assert ((a1 + a2 - ia < 0).sum().item() == 0)
    return ia / (a1 + a2 - ia)
