...
from tools.general.io_utils import create_directory, read_json
from tools.general.time_utils import Timer
from tools.ai.torch_utils import set_seed, calculate_parameters
...
if __name__ == '__main__':
    ...
    model_path = './experiments/models/' + f'{args.tag}.pdparams'
    print(model_path)
    ...
    normalize_fn = transforms.Normalize(mean=imagenet_mean, std=imagenet_std)
    ...
    def get_cam(ori_image, scale):
        ...
        images = paddle.stack([image, flipped_image])
        images = images.cuda()
    ...
    with paddle.no_grad():
        ...
        for step, (ori_image, image_id, label, gt_mask) in enumerate(dataset):
            ...
            strided_cams = paddle.sum(paddle.stack(strided_cams_list), axis=0)
            
            hr_cams_list = [resize_for_tensors(cams.unsqueeze(0), strided_up_size)[0] for cams in cams_list]
            hr_cams = paddle.sum(paddle.stack(hr_cams_list), axis=0)[:, :ori_h, :ori_w]
            
            keys = paddle.nonzero(paddle.to_tensor(label))[:, 0]
            ...
            np.save(npy_path, {"keys": keys, "cam": strided_cams.cpu().numpy(), "hr_cam": hr_cams.cpu().numpy()})
            
            sys.stdout.write('\r# Make CAM [{}/{}] = {:.2f}%, ({}, {})'.format(step + 1, length, (step + 1) / length * 100, (ori_h, ori_w), hr_cams.shape))
            sys.stdout.flush()
        print()
