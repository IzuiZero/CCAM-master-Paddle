...
from paddle.vision import utils
...
def model_info(model, log_func=print):
    ...
    n_g = sum([x.numel() for x in model.parameters() if x.stop_gradient == False])  # number gradients
    log_func('\n%5s %50s %9s %12s %20s %12s %12s' % ('layer', 'name', 'gradient', 'parameters', 'shape', 'mu', 'sigma'))
    for i, (name, p) in enumerate(model.named_parameters()):
        log_func('%5g %50s %9s %12g %20s %12.3g %12.3g' % (
            i, name, not p.stop_gradient, p.numel(), list(p.shape), p.mean().item(), p.std().item()))
...
def accuracy(output, target, topk=(1,)):
    ...
    _, pred = paddle.topk(output, maxk, axis=1)
    pred = paddle.transpose(pred, perm=[1, 0])
    correct = paddle.equal(pred, paddle.expand(target.reshape([1, -1]), shape=pred.shape))
...
def visualize_heatmap(experiments, images, attmaps, epoch, cnt, phase='train', seg=False):
    ...
    for i in range(images.shape[0]):
        ...
        image = grid.multiply(255).add(0.5).clip(0, 255).transpose([1, 2, 0]).astype('uint8').numpy()[..., ::-1]
