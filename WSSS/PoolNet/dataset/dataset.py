...
class VOCImageDataTrain(Dataset):
...
class VOCImageDataTest(Dataset):
...
def get_loader(config, mode='train', pin=False):
    ...
    if mode == 'train':
        ...
        data_loader = paddle.io.DataLoader(dataset=dataset, batch_size=config.batch_size, shuffle=shuffle, num_workers=config.num_thread, use_shared_memory=pin)
    else:
