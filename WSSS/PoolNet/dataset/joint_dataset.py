...
class ImageDataTrain(Dataset):
    ...
    def __getitem__(self, item):
        ...
        edge_image = self.load_image(os.path.join(self.edge_root, edge_im_name))
        edge_label = self.load_edge_label(os.path.join(self.edge_root, edge_gt_name))
        edge_image = paddle.to_tensor(edge_image)
        edge_label = paddle.to_tensor(edge_label)

        # sal data loading
        sal_im_name = self.sal_list[item % self.sal_num].split()[0]
        sal_gt_name = self.sal_list[item % self.sal_num].split()[1]
        sal_image = self.load_image(os.path.join(self.sal_root, sal_im_name))
        sal_label = self.load_sal_label(os.path.join(self.sal_root, sal_gt_name))
        sal_image, sal_label = self.cv_random_flip(sal_image, sal_label)
        sal_image = paddle.to_tensor(sal_image)
        sal_label = paddle.to_tensor(sal_label)
...
def get_loader(config, mode='train', pin=False):
    ...
    if mode == 'train':
        ...
        data_loader = DataLoader(dataset=dataset, batch_size=config.batch_size, shuffle=shuffle, num_workers=config.num_thread, pin_memory=pin)
    else:
