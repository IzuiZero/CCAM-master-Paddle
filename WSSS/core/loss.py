...
def cos_simi(embedded_fg, embedded_bg):
    ...
    sim = paddle.matmul(embedded_fg, embedded_bg, transpose_y=True)
...
class SimMaxLoss(nn.Layer):
    ...
    def forward(self, embedded_bg):
        if self.metric == 'l2':
            ...
            loss = -paddle.log(sim)
            loss = paddle.maximum(loss, paddle.to_tensor(0.0))
            _, indices = paddle.sort(sim, descending=True, axis=1)
            _, rank = paddle.sort(indices, axis=1)
            rank = rank.astype('float32')
            rank_weights = paddle.exp(-rank * self.alpha)
...
if __name__ == '__main__':
    ...
    sim_min_loss_value = sim_min_loss(bg_embedding, fg_embedding)
    print('SimMinLoss:', sim_min_loss_value.numpy())

    sim_max_loss = SimMaxLoss(metric='cos')
    sim_max_loss_value = sim_max_loss(bg_embedding)
    print('SimMaxLoss:', sim_max_loss_value.numpy())
