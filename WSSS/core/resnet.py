...
class ResNet(nn.Layer):
    def __init__(self, block, layers, stride=None, num_classes=1000):
        ...
        for m in self.sublayers():
            if isinstance(m, nn.Conv2D):
                ...
                m.bias.set_value(paddle.zeros(m.weight.shape))
...
def resnet18(pretrained=False, stride=None, num_classes=1000):
    ...
    if stride is None:
        stride = [1, 2, 2, 1]
    model = ResNet(BasicBlock, [2, 2, 2, 2], stride=stride, num_classes=num_classes)
    if pretrained:
        state_dict = load(model_urls['resnet18'])
        model.set_state_dict(state_dict)
    model.fc = nn.Linear(512 * BasicBlock.expansion, num_classes)
    return model

def resnet34(pretrained=False, stride=None, num_classes=1000):
    ...
    if stride is None:
        stride = [1, 2, 2, 1]
    model = ResNet(BasicBlock, [3, 4, 6, 3], stride=stride, num_classes=num_classes)
    if pretrained:
        state_dict = load(model_urls['resnet34'])
        model.set_state_dict(state_dict)
    model.fc = nn.Linear(512 * BasicBlock.expansion, num_classes)
    return model

def resnet50(pretrained=False, stride=None, num_classes=1000):
    ...
    if stride is None:
        stride = [1, 2, 2, 1]
    model = ResNet(Bottleneck, [3, 4, 6, 3], stride=stride, num_classes=num_classes)
    if pretrained:
        state_dict = load(model_urls['resnet50'])
        model.set_state_dict(state_dict)
    model.fc = nn.Linear(512 * Bottleneck.expansion, num_classes)
    return model

def resnet101(pretrained=False, stride=None, num_classes=1000):
    ...
    if stride is None:
        stride = [1, 2, 2, 1]
    model = ResNet(Bottleneck, [3, 4, 23, 3], stride=stride, num_classes=num_classes)
    if pretrained:
        state_dict = load(model_urls['resnet101'])
        model.set_state_dict(state_dict)
    model.fc = nn.Linear(512 * Bottleneck.expansion, num_classes)
    return model

def resnet152(pretrained=False, stride=None, num_classes=1000):
    ...
    if stride is None:
        stride = [1, 2, 2, 1]
    model = ResNet(Bottleneck, [3, 8, 36, 3], stride=stride, num_classes=num_classes)
    if pretrained:
        state_dict = load(model_urls['resnet152'])
        model.set_state_dict(state_dict)
    model.fc = nn.Linear(512 * Bottleneck.expansion, num_classes)
    return model
